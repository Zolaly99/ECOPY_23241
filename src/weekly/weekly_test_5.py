...
def unique_items_over_ten_dollars(input_df):
    list_ = ["item_name", "choice_description", "item_price"]
    df2 = input_df[list_].drop_duplicates()
    df3 = df2[df2["item_price"] > 10]
    return df3[list_].reset_index(drop=True)
...
def minmaxmean_price_in_quartile(input_df):
    input_df2 = generate_quartile(input_df.copy())
    input_df3 = input_df2.groupby('Quartile').agg({'item_price': ['min', 'max', 'mean']}).reset_index()["item_price"]
    return input_df3
